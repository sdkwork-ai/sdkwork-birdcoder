...
assert.match(
  source,
  /getBirdCoderGlobalTokenManager\(\)\.getTokens\(\)/u,
  'Runtime server session headers must read auth/access tokens from the global TokenManager instead of a parallel credential path.',
);
