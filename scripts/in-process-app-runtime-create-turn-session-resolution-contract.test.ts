...
const { createBirdCoderInProcessAppRuntimeTransport } = await import(
  `${appRuntimeTransportModulePath.href}?t=${Date.now()}`
);
const {
...
await appClient.listCodingSessions({ workspaceId: 'workspace-cached' });

const turn = await appClient.createCodingSessionTurn(targetSession.id, {
  requestKind: 'chat',
...
console.log('in-process app runtime create-turn session resolution contract passed.');
