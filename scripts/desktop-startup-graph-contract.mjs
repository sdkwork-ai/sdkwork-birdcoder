...
function resolveListeningPort(server) {
  ...
  assert.ok(
    ...
    'Desktop startup graph contract must listen on a concrete loopback port.',
  );
  return address.port;
}

function createRequestPathNormalizer({ getServerOrigin }) {
  return function normalizeRequestPath(specifier, importerPath) {
...
function collectImports(code, importerPath, normalizeRequestPath) {
...
export async function runDesktopStartupGraphContract({
  host = '127.0.0.1',
  port = 0,
  strictPort = false,
  mode = 'development',
  ...
  const normalizeRequestPath = createRequestPathNormalizer({ getServerOrigin });
  ...
  try {
    ...
    while (queue.length > 0 && visited.size < maxVisited) {
      ...
      for (const dependencyPath of collectImports(source, requestPath, normalizeRequestPath)) {
