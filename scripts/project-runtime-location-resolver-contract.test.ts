...
const desktopIdentityFileSystem = createFileSystemService({});
const desktopIdentityInputs: Array<{
  absolutePath: string;
  projectId: string;
}> = [];
const desktopIdentityResolver = new RuntimeProjectRuntimeLocationService({
  fileSystemService: desktopIdentityFileSystem.fileSystemService,
  identityPort: {
    async resolveDesktopRuntimeLocationBinding(input) {
      desktopIdentityInputs.push(input);
      return {
        displayName: 'desktop-project',
        locationKind: 'local_directory' as const,
        pathFlavor: 'windows' as const,
        rootLocator: 'runtime-location-desktop-1',
        runtimeTargetId: 'desktop-device-1',
        runtimeTargetKind: 'desktop' as const,
      };
    },
  },
});
assert.deepEqual(
  await desktopIdentityResolver.bindLocalProjectRuntimeLocation('project-desktop', {
  ...
  'A durable desktop binding exposes only the opaque runtime location identity.',
);
assert.deepEqual(
  desktopIdentityInputs,
  [{
    absolutePath: 'E:\\work\\desktop-project',
    projectId: 'project-desktop',
  }],
  'Only the dedicated desktop identity boundary may receive the absolute path.',
);
assert.deepEqual(
  await desktopIdentityResolver.resolveProjectRuntimeLocation('project-desktop', {
  ...
  'A later terminal resolution uses the durable local root and exposes only the opaque identifier.',
);

const browserFileSystem = createFileSystemService({});
let browserIdentityCalls = 0;
...
assert.equal(browserIdentityCalls, 0, 'browser handles must not enter desktop identity resolution.');
