...
$mainRs = Join-Path $rootDir "sdkwork-api-birdcoder-standalone-gateway\src\main.rs"
if (-not (Test-Path $mainRs)) {
    Set-Content -Path $mainRs -Value "fn main() {`n    println!(`"sdkwork-api-birdcoder-standalone-gateway`");`n}`n"
}
