...
try {
  ...
  assert.deepEqual(
    ...
    'SQLite console project parent metadata must preserve nested Long ids as strings through SQL JSON storage.',
  );
  assert.deepEqual(
    ...
    'SQLite console project cover image metadata must preserve nested Long ids as strings through SQL JSON storage.',
  );
  ...
  const reloadedRepositories = createBirdCoderConsoleRepositories({
    providerId: 'sqlite',
...
console.log('sqlite console repository contract passed.');
