...
function collectHttpOperations(document, apiPrefix) {
  ...
  for (const [pathKey, methodMap] of Object.entries(document.paths ?? {})) {
    ...
    for (const [methodKey, operation] of Object.entries(methodMap ?? {})) {
      ...
      operations.set(`${tag}.${operation.operationId}`, {
        methodKey,
        operation,
        pathKey,
        tag,
...
function assertReadmesAndGeneratedOutputs() {
  ...
  for (const expected of expectedSurfaces) {
    ...
    if (expected.surface === 'backend') {
      ...
      assert.match(
        tsTypesIndexSource,
        /export interface IamTeamGovernanceListQuery extends Record<string, BirdcoderSdkQueryValue> \{[\s\S]*workspaceId\?: string;[\s\S]*\}/u,
        'backend SDK must preserve referenced team governance query parameters.',
      );
    }
