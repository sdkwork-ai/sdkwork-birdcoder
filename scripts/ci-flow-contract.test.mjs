...
assert.equal(
  ...
  'Root quality scripts must expose project inventory mirror, authority project mirror sync, native session id inventory, lazy transcript, store and render identity deduplication, nonblocking store, workspace/project loading timeout resilience, workspace realtime client resilience, workspace realtime subscriber limits, refresh cache bypass, search performance, parallel mirror behavior, session mirror fallback, imported-only list scoping, and user-scoped local project/workspace fallback as one first-class project loading standard.',
);
assert.equal(
