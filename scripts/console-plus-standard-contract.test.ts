...
assert.notEqual(
  ...
  'console createProject must persist Java-unique studio_project.name values for repeated display names.',
);
assert.notEqual(
...
assert.equal(
  ...
  'Repeated Folder',
  'console title must preserve the requested display name when studio_project.name is made unique.',
);
assert.equal(
  repeatedNameProjectB.title,
  'Repeated Folder',
  'console title must preserve the requested display name for duplicate display-name imports.',
);
assert.ok(
  repeatedNameProjectA.code && repeatedNameProjectA.code.length <= 64,
  'console generated studio_project.code must respect the Java length=64 standard.',
);
assert.ok(
