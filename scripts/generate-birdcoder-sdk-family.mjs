...
${tagGroups.map(([tag]) => `export * from './${tagApiFileName(tag)}';`).join('\n')}
`;
}
...
function renderTypescriptSdkSource(plan, tagGroups) {
  ...
  const apiImports = tagGroups
    .map(([tag]) => `import { ${tagApiFactoryName(tag)}, type ${tagApiName(tag)} } from './api/${tagApiFileName(tag)}';`)
    .join('\n');
...
import { ${operationIndexName} } from './types';
${apiImports}
...
export { createBirdcoder${namePart}SdkClient } from './sdk';
export type {
  Birdcoder${namePart}SdkClient,
...
function generateSurface(plan, { check = false, rootDir = process.cwd() } = {}) {
  ...
  for (const [relativePath, content] of generateTypescriptSources(plan, document, operations)) {
    writeTextFile(path.join(toAbsolutePath(rootDir, plan.typescriptOutput), relativePath), content, {
      check,
      mismatches,
    });
  }

  for (const [relativePath, content] of generateRustSources(plan, operations)) {
    writeTextFile(path.join(toAbsolutePath(rootDir, plan.rustOutput), relativePath), content, {
      check,
      mismatches,
    });
  }
