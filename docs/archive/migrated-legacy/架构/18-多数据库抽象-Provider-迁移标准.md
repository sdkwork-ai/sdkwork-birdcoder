...
- `deployment_target` 现已物化到 `deployment_targets` provider table，并通过 shared repository/query、`/backend/v3/api/projects/:projectId/deployment_targets`、以及 provider/demo/sqlite host route 统一读取。
...
- IAM 主模型和主迁移继续由 SDKWork IAM 标准约束，BirdCoder 只维护非 IAM 业务扩展表与投影表。
