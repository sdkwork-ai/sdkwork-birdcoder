> Migrated from `docs/step/17Q-App-Level-Coding-Session-Projection-Consumer-Adoption-Lane.md` on 2026-06-24.
...
Adopt the implemented app runtime projection read SDK facade into one default app-level coding-session detail consumer boundary without reopening not-yet-promoted app runtime writes or rebuilding projection request paths locally.
...
- `CP17Q-1` app-level coding-session detail reads must stay on `IAppRuntimeReadService`, which already fronts the shared `BirdCoderAppRuntimeReadSdkApiClient`.
...
2. Keep not-yet-promoted app runtime writes outside any shared high-level facade until the corresponding typed facade is closed.
