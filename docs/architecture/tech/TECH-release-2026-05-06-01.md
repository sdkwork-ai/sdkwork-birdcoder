...
- `node scripts/run-cargo.mjs test --manifest-path crates/sdkwork-birdcoder-standalone-gateway/Cargo.toml fake_codex_path_prepend_preserves_existing_path_entries`
- `node scripts/run-cargo.mjs test --manifest-path crates/sdkwork-birdcoder-standalone-gateway/Cargo.toml create_coding_session_turn_route_deduplicates_streamed_terminal_failure_events`
