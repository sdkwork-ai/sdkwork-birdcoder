...
- OpenAPI contract: **162 operations** (product routers + federated `sdkwork-iam` app/backend routers + commerce gateway routes + commerce transactions + chat routes); **162 of 162 implemented**, **0 deferred**. Defer registry: `specs/coding-server-openapi-rust-defer-registry.json` (regenerate with `node scripts/build-coding-server-openapi-rust-defer-registry.mjs`); see `TECH-2026-06-24-commercial-readiness-alignment.md` §2b, §11f, and §15.
