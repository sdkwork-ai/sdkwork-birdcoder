...
## 34. Commercial Readiness Alignment (2026-06-24)

- PC session/auth closure is now code-backed:
  - hash auth redirect to `/#/auth/login`
  - structured HTTP `401` via `BirdCoderApiTransportError`
  - proactive IAM refresh through `auth.sessions.refresh`
- Rust host now serves live OpenAPI at `/openapi.json` aligned with packaged `coding-server-v1.json` snapshots.
- Workspace realtime clients reconnect with exponential backoff before giving up.
- Operator runbooks live under `docs/guides/operator/`; commercial truth doc is `TECH-2026-06-24-commercial-readiness-alignment.md`.
- OpenAPI contract: **147 operations** (product routers + federated `sdkwork-iam` app/backend routers); **132 of 147 implemented**, **15 commerce pre-launch deferred** (`/api/v1/api-keys`, `/api/v1/notifications`, `/api/v1/usage`). Defer registry: `specs/coding-server-openapi-rust-defer-registry.json` (regenerate with `node scripts/build-coding-server-openapi-rust-defer-registry.mjs`); see `TECH-2026-06-24-commercial-readiness-alignment.md` §2b for the commerce pre-launch deferred lane policy.
- Mobile CI: H5 typecheck/build, Capacitor `cap:sync`, Android `assembleDebug` (`pnpm cap:android:assemble`), Flutter analyze/test in `mobile-surfaces` job.
- Pre-launch manifest: install packages remain disabled until first governed **production** release artifacts ship with real checksums/SBOM evidence (synthetic rehearsal via `release:fixture:ready` / `release:candidate:dry-run` is aligned).
- Mandatory verification for this slice:
  ...
  - `cargo test -p sdkwork-birdcoder-standalone-gateway openapi`
