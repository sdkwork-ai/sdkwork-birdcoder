> Migrated from `docs/release/release-2026-04-13-08.md` on 2026-06-24.
> Owner: SDKWork maintainers

## Highlights

- Closes Step `20B deployment_target` by promoting `deployment_target` from schema-only definition into shared repository/query truth, shared app/backend facade truth, real `coding-server` route truth, and Rust host demo/sqlite authority truth.
- Freezes `/backend/v3/api/projects/:projectId/deployment_targets` as the canonical admin deployment-target read surface across TS route contracts, generated client requests, in-process transport, and the Rust host OpenAPI matrix.
...
- `cargo test --manifest-path crates/sdkwork-birdcoder-standalone-gateway/Cargo.toml -- --nocapture`
