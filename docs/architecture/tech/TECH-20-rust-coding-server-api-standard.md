> Migrated from `docs/架构/20-统一Rust-Coding-Server-API-协议标准.md` on 2026-06-24.
...
- `apps/sdkwork-birdcoder-pc/packages/sdkwork-birdcoder-pc-server/src-host`
  - Rust host 进程、最小 app/backend 路由骨架与未来真实 handler 承载层
...
- `app`: Runtime, Engine, Coding Session, Turn, Event, Artifact, Checkpoint, Approval, Operation, Workspace, Project, Document, Template, Prompt, Skill, Build/Test/Preview/Deployment
- `backend`: Team, Member, Policy, Audit, Release, Deployment Governance
...
已在 `apps/sdkwork-birdcoder-pc/packages/sdkwork-birdcoder-pc-server/src/index.ts` 落地：
...
已在 `apps/sdkwork-birdcoder-pc/packages/sdkwork-birdcoder-pc-server/src/projectionRepository.ts` 落地：
...
已在 `crates/sdkwork-birdcoder-standalone-gateway/src/lib.rs` 落地：
...
- representative `app / backend` OpenAPI path matrix
- representative list handlers for `app / backend` now return unified SdkWork list envelopes (`data.items` + `data.pageInfo`) on `/app/v3/api/engines`, `/app/v3/api/workspaces`, and `/backend/v3/api/releases`
- projection-backed read handlers now serve `/app/v3/api/coding_sessions/:id`, `/app/v3/api/operations/:operationId`, `/app/v3/api/coding_sessions/:id/events`, `/app/v3/api/coding_sessions/:id/artifacts`, and `/app/v3/api/coding_sessions/:id/checkpoints`
...
- representative real app/backend handlers now serve `/app/v3/api/projects`, `/app/v3/api/teams`, `/backend/v3/api/iam/teams`, and `/backend/v3/api/releases` from the same runtime authority state, returning list envelopes instead of placeholder or empty-shell payloads
- Desktop `src-tauri` now materializes the first real direct authority tables for coding-session and representative app/backend resources inside the shared SQLite file, instead of leaving `kv_store` as the only persisted authority shape
...
- `workspace/project/team/release_record` storage binding for representative app/backend authority paths
- `packages/sdkwork-birdcoder-types/src/server-api.ts` freezes shared `BIRDCODER_CODING_SERVER_API_VERSION`, `BirdCoderApiTransport`, and timestamped `project` summary DTOs so app/backend consumers no longer need app-local DTO shims
- `packages/sdkwork-birdcoder-infrastructure/src/services/sdkClients.ts` exposes only the explicit `BirdCoderAppSdkApiClient` and `BirdCoderBackendSdkApiClient` wrappers on top of the generated app/backend SDK packages
...
- representative app/backend direct authority tables and TS migration bundles no longer diverge on those two entities
...
- `packages/sdkwork-birdcoder-infrastructure/src/storage/appConsoleRepository.ts` now closes the representative `project / team / release_record` repository set on the same provider/UoW plus row/plan contract already used by `coding-server` projection persistence.
- Shared entity definitions now mark nullable runtime and representative app/backend columns explicitly so fresh SQLite schema, Rust authority tables, and TS repositories no longer rely on JSON-fallback-only null assumptions.
- `packages/sdkwork-birdcoder-infrastructure/src/services/sdkClients.ts` now closes the typed app/backend SDK surface with explicit app and backend clients plus in-process and HTTP transport adapters.
- Mixed split-client wrappers are not part of the standard surface; consumers compose the app SDK client and backend SDK client explicitly.
- `createDefaultBirdCoderIdeServices()` now composes `ApiBackedWorkspaceService` and `ApiBackedProjectService`; default workspace/project catalog reads flow through the app SDK client, while local writes and coding-session in-memory state remain on the existing provider-backed sidecars.
...
- `POST /app/v3/api/coding_sessions`
- `GET /app/v3/api/coding_sessions/:id`
- `POST /app/v3/api/coding_sessions/:id/turns`
...
- `POST /app/v3/api/approvals/:approvalId/decision`
- `GET /app/v3/api/operations/:operationId`

## 6. 返回模型标准

HTTP JSON 成功响应统一使用 `SdkWorkApiResponse`（`API_SPEC.md` section 15）：

- 顶层：`code`（成功固定为 `0`）、`data`、`traceId`
- 单体资源：`data.item`
- 列表资源：`data.items` + `data.pageInfo`（`PageInfo.mode` 为 `offset` 或 `cursor`）
- 命令结果：`data.accepted`，可选 `resourceId` / `status`
- 异步受理（`202`）：`data.operationId`、`data.status`，可选 `pollUrl`

HTTP 错误统一使用 `application/problem+json`（`ProblemDetail`）：

- 必填 numeric `code`（平台错误码，例如 `40401`）与 `traceId`
- 禁止在 HTTP 2xx 中返回非零 `code`、字符串 wire code、`success` 或 human `message`

SSE / 流式事件 chunk 与 HTTP JSON 共用同一 `SdkWorkApiResponse` 外壳；`traceId` 由服务端生成，客户端不得伪造。
...
3. Canonical OpenAPI export、finalized release evidence、generated OpenAPI types、generated client types、shared `app / backend` SDK facades and app runtime facades 已在本页 `13` 到 `32` 节闭环。
...
- `BootstrapShellRuntimeOptions.host.apiBaseUrl` is the preferred source for shared app/backend client bootstrap on web / desktop; explicit `apiBaseUrl` or `appClient/backendClient` override it only when a host needs a non-default transport.
- `createDefaultBirdCoderIdeServices()` now resolves reads in this order:
  1. explicit `appClient/backendClient`
  2. runtime-configured `appClient/backendClient`
...
- HTTP transport URL join now preserves any path prefix already present in `apiBaseUrl`. Example:
  ...
  - route: `/app/v3/api/workspaces`
  - final: `https://cn.sdkwork.local/birdcoder/app/v3/api/workspaces`
- `bindDefaultBirdCoderIdeServicesRuntime()` now owns the shared host-to-transport binding rule, so shell bootstrap and server runtime binding consume one source of truth for `host -> apiBaseUrl -> default app/backend client` adoption.
- `BirdCoderAppSdkApiClient.listTeams()` now standardizes workspace team catalog reads on `/app/v3/api/teams`, while `BirdCoderBackendSdkApiClient.listGovernanceTeams()` remains the explicit backend-surface governance reader on `/backend/v3/api/iam/teams`.
...
- `buildBirdCoderCodingServerOpenApiDocument()` is now the canonical TS-side document builder; `buildBirdCoderCodingServerOpenApiDocument()` is the only standard entrypoint.
...
- `packages/sdkwork-birdcoder-infrastructure/src/services/sdkClients.ts` is now the first real consumer of the generated request builder for representative app/backend reads:
...
- Follow-on closure from this point is now closed across sections `17` through `32`: representative generated-client adoption now sits behind shared high-level facades plus the typed app runtime read/write response surfaces.

## 17. Current Loop Closure - Generated App/Backend SDK Client

- `packages/sdkwork-birdcoder-types/src/server-api.ts` now closes the first explicit high-level app/backend SDK client pair on top of `createBirdCoderFinalizedCodingServerClient()`.
- `createBirdCoderAppSdkApiClient({ transport: appTransport }) and createBirdCoderBackendSdkApiClient({ transport: backendTransport })` is now the only approved explicit app/backend SDK client pair for representative catalog reads on:
  - `/app/v3/api/workspaces`
  - `/app/v3/api/projects`
  - `/app/v3/api/teams`
  - `/backend/v3/api/iam/teams`
  - `/backend/v3/api/releases`
- `packages/sdkwork-birdcoder-infrastructure/src/services/sdkClients.ts` now acts as a thin delegator that contributes transport implementations only; it no longer hand-assembles representative app/backend request paths.
- `scripts/split-sdk-client-facade-contract.test.ts` now proves the explicit app/backend SDK client pair preserves representative method/path/query wiring and unified list-envelope consumption.
- `check:release-flow` now executes that direct app/backend client contract, so this governance rule is executable rather than docs-only.
- Follow-on closure from this point is now closed across sections `18` through `32`: the same direct app/backend client pattern now covers default IDE services plus the representative shared `app / backend` consumer set.

## 18. Current Loop Closure - Default IDE Services App/Backend SDK Adoption

- `packages/sdkwork-birdcoder-infrastructure/src/services/defaultIdeServices.ts` now composes default runtime HTTP clients and in-process fallback clients directly through `createBirdCoderAppSdkApiClient({ transport: appTransport }) and createBirdCoderBackendSdkApiClient({ transport: backendTransport })`.
- The default IDE service layer no longer uses a mixed split SDK wrapper for transport-based representative app/backend reads; no mixed infrastructure wrapper remains in the default composition path.
- `scripts/default-ide-services-split-sdk-client-contract.test.ts` now locks this adoption rule as an executable architecture contract.
- `check:release-flow` now executes that contract beside the existing direct app/backend client contract, so the default service-composition path cannot silently drift back to wrapper-based request assembly.
...
## 19. Current Loop Closure - App/Backend Wrapper Removal

- `packages/sdkwork-birdcoder-infrastructure/src/services/sdkClients.ts` no longer exports a high-level mixed split SDK wrapper or its dedicated options type.
- The infrastructure module now owns transport factories only:
  - `createBirdCoderHttpApiTransport()`
  - `createBirdCoderInProcessAppSdkTransport()/createBirdCoderInProcessBackendSdkTransport()`
- High-level representative app/backend client composition is now owned only by `createBirdCoderAppSdkApiClient({ transport: appTransport }) and createBirdCoderBackendSdkApiClient({ transport: backendTransport })` in `@sdkwork/birdcoder-infrastructure`.
- `scripts/split-sdk-consumer-contract.test.ts` now consumes the explicit app/backend SDK clients directly while preserving the same representative workspace/project/team/release verification path.
- `scripts/birdcoder-sdk-consumer-boundary-contract.test.mjs` now prevents wrapper reintroduction and is part of `check:release-flow`.
- Follow-on closure from this point is now closed across sections `20` through `32`: later app runtime SDK transport consumers now keep infrastructure transport-only while types owns the high-level facades.

## 20. Current Loop Closure - App Runtime SDK Read Facade

- `packages/sdkwork-birdcoder-infrastructure/src/services/sdkClients.ts` now exposes `createBirdCoderAppSdkApiClient({ transport })` as the app runtime read SDK facade on top of `createBirdCoderFinalizedCodingServerClient()`.
- The current facade scope is intentionally limited to representative app runtime routes that already have real server behavior:
  - `GET /app/v3/api/system/descriptor`
  - `GET /app/v3/api/system/runtime`
  - `GET /app/v3/api/system/health`
  - `GET /app/v3/api/engines`
  - `GET /app/v3/api/operations/:operationId`
- The explicit app/backend SDK client pair now standardizes:
...
- `scripts/app-runtime-read-sdk-client-contract.test.ts` now proves request wiring for those five app runtime routes and is part of `check:release-flow`.
- Governance rule:
  - do not expose unimplemented app runtime routes through the shared read facade
  - extend the facade only when the underlying server route has real behavior instead of `not_implemented`
- Follow-on closure from this point is now closed in section `21`: the app runtime SDK facade now covers the implemented projection reads.

## 21. Current Loop Closure - App Runtime SDK Projection Read Facade

- `packages/sdkwork-birdcoder-infrastructure/src/services/sdkClients.ts` now extends `createBirdCoderAppSdkApiClient({ transport })` to the already implemented projection-read routes:
  - `GET /app/v3/api/coding_sessions/:id`
  - `GET /app/v3/api/coding_sessions/:id/events`
  - `GET /app/v3/api/coding_sessions/:id/artifacts`
  - `GET /app/v3/api/coding_sessions/:id/checkpoints`
- The explicit app/backend SDK client pair now standardizes projection-read response mapping as:
...
- `scripts/app-runtime-projection-read-sdk-client-contract.test.ts` now proves method/path wiring for those four projection-read routes and is part of `check:release-flow`.
- Governance rule:
  - keep projection reads on the shared generated-client-based facade once Rust host behavior is real
  - keep not-yet-promoted app runtime writes and still-unimplemented reads outside the shared high-level facade
  - continue keeping transport creation in infrastructure and high-level request assembly in `@sdkwork/birdcoder-types`
- Follow-on closure from this point is now closed across sections `22` through `32`: representative shared transport consumers now stay on the generated-client plus direct app/backend client stack.

## 22. Current Loop Closure - App Team Surface Split

- `packages/sdkwork-birdcoder-types/src/server-api.ts` now splits workspace team reads from backend governance team reads on the shared app/backend facade:
  - `listTeams()` -> `GET /app/v3/api/teams`
  - `listAdminTeams()` -> `GET /backend/v3/api/iam/teams`
- `packages/sdkwork-birdcoder-infrastructure/src/services/sdkClients.ts` now serves both team surfaces in the in-process transport instead of forcing runtime team reads through the backend route.
- `ApiBackedTeamService` and the default IDE service/runtime transport path now consume the app-surface team catalog by default, so standard workspace reads no longer depend on backend-surface governance semantics.
- `scripts/split-sdk-client-facade-contract.test.ts`, `scripts/split-sdk-consumer-contract.test.ts`, and `scripts/server-runtime-transport-contract.test.ts` now lock this split into executable governance.
- Governance rule:
  - runtime workspace/team reads must stay on `/app/v3/api/*`
  - backend governance team management reads must stay explicit via `listAdminTeams()`
  - do not collapse app/backend team semantics back into one ambiguous backend-only method
- Follow-on closure from this point is now closed across sections `23` through `32`: the remaining real transport consumers now preserve the same app/backend split and direct app/backend client adoption rules.
...
- `IReleaseService` and `ApiBackedReleaseService` now close the first representative governed release-read service boundary on top of the shared app/backend facade.
- `createDefaultBirdCoderIdeServices()`, `IDEContext`, `ServiceContext`, and `useReleases()` now expose the same governed release catalog surface to app consumers without rebuilding backend HTTP or DTOs locally.
- `scripts/default-ide-services-release-service-contract.test.ts` is now part of `check:release-flow`.
- Governance rule:
  - keep governed release catalog reads explicit on the backend surface via `listReleases()`
  - keep request assembly in the generated client plus explicit app/backend SDK client pair
  - adopt implemented app runtime SDK reads into default service/context consumers before typed write/response facades

## 24. Current Loop Closure - Default IDE App Runtime Read Adoption

- `IAppRuntimeReadService` and `ApiBackedAppRuntimeReadService` now close the first default IDE service boundary on top of `BirdCoderAppRuntimeReadSdkApiClient`.
- Runtime-bound default IDE services now compose `createBirdCoderAppSdkApiClient({ transport: createBirdCoderHttpApiTransport(...) })` directly.
- `createDefaultBirdCoderIdeServices()`, `IDEContext`, and `ServiceContext` now expose `appRuntimeReadService`; `useCodingServerOverview()` is the first app-level consumer for:
  - `getDescriptor()`
  - `getRuntime()`
  - `getHealth()`
  - `listEngines()`
- This loop does not claim a local in-process app runtime transport closure. Without a bound runtime or injected `appRuntimeReadClient`, app runtime reads must stay explicitly unavailable instead of fabricating local authority.
- `scripts/default-ide-services-app-runtime-read-sdk-contract.test.ts` and `scripts/default-ide-services-app-runtime-read-service-contract.test.ts` are now part of `check:release-flow`.
- Governance rule:
  - keep runtime HTTP app runtime reads on the explicit app/backend SDK clients
  - do not rebuild app runtime request paths in infrastructure or app consumers
  - adopt app runtime projection reads into app consumers next; do not skip forward to not-yet-promoted app runtime writes
...
- Governance rule:
  ...
  - keep app consumers on `appRuntimeReadService` and shared projection payloads only
  - keep not-yet-promoted app runtime writes outside high-level facades until the typed promotion lane closes

## 26. Current Loop Closure - App Runtime SDK Facade Exclusion Governance
...
- `scripts/app-runtime-sdk-facade-governance-contract.test.ts` is now part of `check:release-flow`.
- Governance rule:
...
## 27. Current Loop Closure - Real App Runtime Create Session Route

- Rust host `POST /app/v3/api/coding_sessions` no longer returns `not_implemented`.
- The route now:
...
- `crates/sdkwork-birdcoder-standalone-gateway/src/lib.rs` now keeps projection state on one shared authority handle so create/read requests in the same host process observe the same truth.
...
- Governance rule:
  - treat `codingSessions.create` as server-real but still excluded from the shared high-level facade until the typed write/response facade lane closes
  - do not keep describing `codingSessions.create` as `not_implemented`

## 28. Current Loop Closure - Typed App Runtime Create Session Facade And Consumer Adoption

- `packages/sdkwork-birdcoder-infrastructure/src/services/sdkClients.ts` now exposes `createBirdCoderAppSdkApiClient({ transport })` on top of `createBirdCoderFinalizedCodingServerClient()`.
- The promoted app runtime operation catalog now includes:
  ...
  - `codingSessions.artifacts.list`
  - `codingSessions.checkpoints.list`
- The excluded app runtime operation catalog now keeps only:
  - `engines.capabilities.retrieve`
  - `models.list`
  - `codingSessions.turns.create`
  - `approvals.decisions.create`
...
- `ApiBackedProjectService.createCodingSession()` is now the first real consumer boundary on top of the app runtime write SDK facade:
  - resolve `workspaceId` from project truth
  - call server-authoritative `codingSessions.create`
...
- Governance rule:
  - now that `codingSessions.turns.create` is server-real, keep it outside shared high-level facades until the typed write/response facade plus first consumer adoption close together
...
## 29. Current Loop Closure - Real App Runtime Create Session Turn Route

- Rust host `POST /app/v3/api/coding_sessions/:id/turns` no longer returns `not_implemented`.
- The route now:
...
- `crates/sdkwork-birdcoder-standalone-gateway/src/lib.rs` now keeps turn creation on the same authority boundary as session create/read, so turn create, operation read, and event replay observe one truth source.
- Rust verification now covers both execution modes:
...
- Governance rule:
  - treat `codingSessions.turns.create` as server-real but still excluded from the shared high-level facade until the typed write facade and first consumer path close together
  - do not keep describing `codingSessions.turns.create` as `not_implemented`
  - do not promote `codingSessions.turns.create` into shared governance before release-flow and first consumer adoption are both executable

## 30. Current Loop Closure - Typed App Runtime Create Session Turn Facade And Consumer Adoption

- `packages/sdkwork-birdcoder-infrastructure/src/services/sdkClients.ts` now extends `createBirdCoderAppSdkApiClient({ transport })` with `createCodingSessionTurn(codingSessionId, request)`.
- The promoted app runtime operation catalog now includes:
  - `codingSessions.turns.create`
  - `codingSessions.create`
  - the existing implemented app runtime read operations
- The excluded app runtime operation catalog now keeps only:
  - `engines.capabilities.retrieve`
  - `models.list`
  - `approvals.decisions.create`
- `createBirdCoderAppSdkApiClient({ transport })` now normalizes:
  - `codingSessionId`
  - `requestKind`
  - `inputSummary`
  - optional `runtimeId`
...
- Governance rule:
  - keep turn-write request assembly in the generated client plus app runtime write SDK facade
...
## 31. Current Loop Closure - Real App Runtime Engine Capability And Model Catalog Adoption

- Rust host `GET /app/v3/api/engines`, `GET /app/v3/api/engines/:engineKey/capabilities`, and `GET /app/v3/api/models` now return real canonical engine/model truth instead of placeholder envelopes.
- The canonical server catalog now covers the current engine/kernel baseline:
  - `codex`
  - `claude-code`
  - `gemini`
  - `opencode`
- `packages/sdkwork-birdcoder-types/src/server-api.ts` now extends the app runtime read SDK facade with:
  - `getEngineCapabilities(engineKey)`
  - `listModels()`
- The promoted app runtime operation catalog now also includes:
  - `engines.capabilities.retrieve`
  - `models.list`
- The excluded app runtime operation catalog now keeps only:
  - `approvals.decisions.create`
- `IAppRuntimeReadService`, `ApiBackedAppRuntimeReadService`, and `createDefaultBirdCoderIdeServices()` now expose the promoted engine/model reads end-to-end.
...
- Governance rule:
  ...
  - keep `approvals.decisions.create` blocked until approval authority truth plus consumer adoption are both real
  - treat this section as the current authoritative override for engine/model app-runtime SDK governance

## 32. Current Loop Closure - Real App Runtime Approval Decision Lane

- Rust host `POST /app/v3/api/approvals/:approvalId/decision` now returns a real approval-decision envelope instead of `not_implemented`.
...
- `packages/sdkwork-birdcoder-types/src/server-api.ts` now extends the app runtime write SDK facade with `submitApprovalDecision(approvalId, request)`.
- app runtime SDK high-level governance is now fully promoted for currently real app runtime routes:
  - `BIRDCODER_APP_RUNTIME_SDK_OPERATION_IDS` includes `approvals.decisions.create`
  - `BIRDCODER_APP_RUNTIME_SDK_EXCLUDED_OPERATION_IDS` is now empty
- `IAppRuntimeWriteService`, `ApiBackedAppRuntimeWriteService`, default IDE service composition, `IDEContext`, and `ServiceContext` now expose approval submission through one shared write boundary instead of page-local route assembly.
...
- Executable verification for this closure is:
  ...
  - `pnpm.cmd run typecheck`
- Follow-on closure from this point is now closed in sections `33` through `38`: the remaining representative app/backend routes no longer return `not_implemented`, and the PostgreSQL host-pass closure is recorded later on this page.

## 33. Current Loop Closure - Real App Document Catalog Lane

- Rust host `GET /app/v3/api/documents` now returns a real document catalog envelope instead of `not_implemented`.
...
- `packages/sdkwork-birdcoder-types/src/server-api.ts` now extends the shared app/backend facade with `listDocuments()`.
- `packages/sdkwork-birdcoder-infrastructure/src/storage/appConsoleRepository.ts`, `consoleQueries.ts`, and `sdkClients.ts` now promote `project_documents` through the shared repository/query/transport boundary.
- `IDocumentService`, `ApiBackedDocumentService`, `createDefaultBirdCoderIdeServices()`, `IDEContext`, `ServiceContext`, `loadDocuments()`, and `useDocuments()` now close the first reusable document consumer boundary on the explicit app/backend SDK client pair path.
- Executable verification for this closure is:
...
- Remaining representative placeholder routes at this checkpoint were:
  ...
  - `GET /backend/v3/api/iam/audit_events`
...
- Rust host `GET /backend/v3/api/iam/audit_events` now returns a real audit catalog envelope instead of `not_implemented`.
...
- `packages/sdkwork-birdcoder-types/src/server-api.ts` now extends the shared app/backend facade with `listAuditEvents()`.
- `packages/sdkwork-birdcoder-infrastructure/src/storage/appConsoleRepository.ts`, `consoleQueries.ts`, and `sdkClients.ts` now promote `audit_events` through the shared repository/query/transport boundary.
- `IAuditService`, `ApiBackedAuditService`, `createDefaultBirdCoderIdeServices()`, `IDEContext`, `ServiceContext`, `loadAuditEvents()`, and `useAuditEvents()` now close the first reusable audit consumer boundary on the explicit app/backend SDK client pair path.
- Executable verification for this closure is:
  - `cargo test --manifest-path crates/sdkwork-birdcoder-standalone-gateway/Cargo.toml representative_app_and_admin_real_list_routes_return_runtime_data`
  - `cargo test --manifest-path crates/sdkwork-birdcoder-standalone-gateway/Cargo.toml build_app_loads_projection_state_from_sqlite_kv_store_when_configured`
  - `cargo test --manifest-path crates/sdkwork-birdcoder-standalone-gateway/Cargo.toml build_app_loads_projection_state_from_direct_sqlite_provider_tables_when_configured`
...
- Rust host `GET /app/v3/api/deployments` now returns a real deployment catalog envelope instead of `not_implemented`.
...
- `packages/sdkwork-birdcoder-types/src/server-api.ts` now extends the shared app/backend facade with `listDeployments()`.
- `packages/sdkwork-birdcoder-infrastructure/src/storage/appConsoleRepository.ts`, `consoleQueries.ts`, and transport-backed app/backend client wiring now promote `deployment_records` through the shared repository/query/transport boundary.
- `IDeploymentService`, `ApiBackedDeploymentService`, `createDefaultBirdCoderIdeServices()`, `IDEContext`, `ServiceContext`, `loadDeployments()`, and `useDeployments()` now close the first reusable deployment consumer boundary on the explicit app/backend SDK client pair path.
- Executable verification for this closure is:
...
- Rust host `GET /backend/v3/api/deployments` now returns a real deployment governance envelope instead of `not_implemented`.
- Representative backend deployment truth now converges on one replayable authority path:
  - demo host: in-process deployment state
  - legacy sqlite `kv_store`: deployment payloads materialized into `deployment_records`
  - direct sqlite provider: `deployment_records`
- `packages/sdkwork-birdcoder-types/src/server-api.ts` now extends the shared app/backend facade with `listAdminDeployments()`.
...
- `IAdminDeploymentService`, `ApiBackedAdminDeploymentService`, `createDefaultBirdCoderIdeServices()`, `IDEContext`, `ServiceContext`, `loadAdminDeployments()`, and `useAdminDeployments()` now close the first reusable backend deployment consumer boundary on the explicit app/backend SDK client pair path.
- Executable verification for this closure is:
  - `cargo test --manifest-path crates/sdkwork-birdcoder-standalone-gateway/Cargo.toml representative_app_and_admin_real_list_routes_return_runtime_data -- --nocapture`
  - `cargo test --manifest-path crates/sdkwork-birdcoder-standalone-gateway/Cargo.toml build_app_loads_projection_state_from_sqlite_kv_store_when_configured -- --nocapture`
  - `cargo test --manifest-path crates/sdkwork-birdcoder-standalone-gateway/Cargo.toml build_app_loads_projection_state_from_direct_sqlite_provider_tables_when_configured -- --nocapture`
...
- Remaining representative placeholder routes at this checkpoint were:
  - `GET /backend/v3/api/iam/policies`
- The follow-on serial closure from this point was the dedicated backend policy governance lane, now closed in section `37`.

## 37. Current Loop Closure - Real Admin Policy Governance Lane

- Rust host `GET /backend/v3/api/iam/policies` now returns a real governance-policy envelope instead of `not_implemented`.
- Representative backend policy truth now converges on one dedicated authority path:
...
- `packages/sdkwork-birdcoder-types/src/server-api.ts` now extends the shared app/backend facade with `listPolicies()`.
- `packages/sdkwork-birdcoder-infrastructure/src/storage/appConsoleRepository.ts`, `consoleQueries.ts`, and `sdkClients.ts` now promote `governance_policies` through the shared repository/query/transport boundary.
- `IAdminPolicyService`, `ApiBackedAdminPolicyService`, `createDefaultBirdCoderIdeServices()`, `IDEContext`, `ServiceContext`, `loadAdminPolicies()`, and `useAdminPolicies()` now close the first reusable backend policy consumer boundary on the explicit app/backend SDK client pair path.
- Executable verification for this closure is:
  - `pnpm.cmd run test:split-sdk-client-facade-contract`
  - `pnpm.cmd run test:split-sdk-consumer-contract`
