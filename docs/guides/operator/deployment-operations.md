...
- Contract route count: **132 operations** (product routers + federated `sdkwork-iam` app/backend routers in api-server)
- Defer registry: `specs/coding-server-openapi-rust-defer-registry.json` — **132 of 132 implemented**, **0 deferred** (regenerate with `node scripts/build-coding-server-openapi-rust-defer-registry.mjs`)
