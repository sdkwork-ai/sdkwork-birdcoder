...
The release standard is not 鈥渄ifferent API per deployment.鈥?It is 鈥渄ifferent server binding behind the same BirdCoder facade.鈥?Packaged desktop and web clients still consume the same canonical `/api/app/v1/auth/*`, `/api/app/v1/user/profile`, and `/api/app/v1/vip/info` routes after packaging.
