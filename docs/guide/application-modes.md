...
| `external-user-center` | `pnpm dev:external`, `pnpm tauri:dev:external`, `pnpm desktop:dev:external`, `pnpm web:dev:external`, `pnpm server:dev:external` | BirdCoder server keeps `/app/v3/api/auth/*` and related facade routes but resolves IAM from a third-party bridge |
