...
- representative real app/backend handlers now serve `/api/app/v1/projects`, `/api/app/v1/teams`, `/api/admin/v1/teams`, and `/api/admin/v1/releases` from the same runtime authority state, returning list envelopes instead of placeholder or empty-shell payloads
...
- `packages/sdkwork-birdcoder-types/src/server-api.ts` now freezes shared `BIRDCODER_CODING_SERVER_API_VERSION`, `BirdCoderApiTransport`, `BirdCoderSplitSdkApiClients`, and timestamped `project` summary DTOs so app/backend consumers no longer need app-local DTO shims
- `packages/sdkwork-birdcoder-types/src/server-api.ts` now also exposes `createBirdCoderSplitSdkApiClients()` as the shared high-level representative app/backend facade on top of the release-backed generated client
...
- `packages/sdkwork-birdcoder-infrastructure/src/services/sdkClients.ts` now closes the first typed app/backend SDK surface with one shared client plus in-process and HTTP transport adapters.
- `packages/sdkwork-birdcoder-infrastructure/src/services/sdkClients.ts` now delegates high-level representative app/backend request assembly to `createBirdCoderSplitSdkApiClients()` and keeps infrastructure scoped to transport concerns only.
- `createDefaultBirdCoderIdeServices()` now composes `ApiBackedWorkspaceService` and `ApiBackedProjectService`; default workspace/project catalog reads flow through the shared app/backend client, while local writes and coding-session in-memory state remain on the existing provider-backed sidecars.
...
- `BirdCoderAppSdkApiClient.listTeams()` now standardizes workspace team catalog reads on `/api/app/v1/teams`, while `BirdCoderBackendSdkApiClient.listGovernanceTeams()` remains the explicit admin-surface reader on `/api/admin/v1/teams`.
...
- `packages/sdkwork-birdcoder-types/src/server-api.ts` now closes the first shared high-level representative app/backend facade on top of `createBirdCoderFinalizedCodingServerClient()`.
- `createBirdCoderSplitSdkApiClients({ appTransport, backendTransport })` is now the only approved shared facade for representative catalog reads on:
  - `/api/app/v1/workspaces`
  - `/api/app/v1/projects`
  - `/api/app/v1/teams`
  - `/api/admin/v1/teams`
...
- `scripts/generated-app/backend-client-facade-contract.test.ts` now proves the shared facade preserves representative method/path/query wiring and unified list-envelope consumption.
...
- `packages/sdkwork-birdcoder-infrastructure/src/services/defaultIdeServices.ts` now composes default runtime HTTP clients and in-process fallback clients directly through `createBirdCoderSplitSdkApiClients({ appTransport, backendTransport })`.
- The default IDE service layer no longer uses `createBirdCoderSplitSdkApiClients()` for transport-based representative app/backend reads; the infrastructure wrapper remains optional compatibility glue, not the default composition path.
- `scripts/default-ide-services-generated-app/backend-facade-contract.test.ts` now locks this adoption rule as an executable architecture contract.
...
- `packages/sdkwork-birdcoder-infrastructure/src/services/sdkClients.ts` no longer exports a high-level `createBirdCoderSplitSdkApiClients()` wrapper or its dedicated options type.
...
- High-level representative app/backend client composition is now owned only by `createBirdCoderSplitSdkApiClients({ appTransport, backendTransport })` in `@sdkwork/birdcoder-types`.
- `scripts/app/backend-sdk-consumer-contract.test.ts` now consumes the shared generated facade directly while preserving the same representative workspace/project/team/release verification path.
- `scripts/no-app/backend-client-wrapper-contract.test.ts` now prevents wrapper reintroduction and is part of `check:release-flow`.
...
- `scripts/app-runtime-read-sdk-client-contract.test.ts` now proves request wiring for those five core routes and is part of `check:release-flow`.
- Governance rule:
...
- `packages/sdkwork-birdcoder-types/src/server-api.ts` now splits workspace team reads from admin team reads on the shared app/backend facade:
  - `listTeams()` -> `GET /api/app/v1/teams`
  - `listAdminTeams()` -> `GET /api/admin/v1/teams`
- `packages/sdkwork-birdcoder-infrastructure/src/services/sdkClients.ts` now serves both team surfaces in the in-process transport instead of forcing runtime team reads through the admin route.
...
- `scripts/generated-app/backend-client-facade-contract.test.ts`, `scripts/app/backend-sdk-consumer-contract.test.ts`, and `scripts/server-runtime-transport-contract.test.ts` now lock this split into executable governance.
- Governance rule:
  - runtime workspace/team reads must stay on `/api/app/v1/*`
  - admin team management reads must stay explicit via `listAdminTeams()`
  - do not collapse app/backend team semantics back into one ambiguous admin-only method
- Follow-on closure from this point is now closed across sections `23` through `32`: the remaining real transport consumers now preserve the same app/backend split and shared-facade adoption rules.
...
- `scripts/default-ide-services-app-runtime-read-sdk-contract.test.ts` and `scripts/default-ide-services-core-read-service-contract.test.ts` are now part of `check:release-flow`.
- Governance rule:
