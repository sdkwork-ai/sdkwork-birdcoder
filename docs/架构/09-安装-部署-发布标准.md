...
- Governance rule:
  ...
  - do not fork app/backend/core request assembly per host
...
- `@sdkwork/birdcoder-types` owns `createBirdCoderSplitSdkApiClients({ appTransport, backendTransport })` as the shared high-level representative app/backend facade.
...
- `node --experimental-strip-types scripts/generated-app/backend-client-facade-contract.test.ts` is the executable contract for this standard.
...
- `BirdCoderAppSdkApiClient.listTeams()` is the workspace/runtime team reader and must resolve against `/api/app/v1/teams`.
- `BirdCoderBackendSdkApiClient.listGovernanceTeams()` is the explicit admin team reader and must resolve against `/api/admin/v1/teams`.
...
- Governance rule:
  ...
  - preserve one shared facade while keeping app/backend team semantics explicit

## 16. Default IDE Services Shared-Facade Adoption Standard

- `packages/sdkwork-birdcoder-infrastructure/src/services/defaultIdeServices.ts` must compose runtime HTTP and in-process fallback app/backend clients directly through `createBirdCoderSplitSdkApiClients({ appTransport, backendTransport })`.
- Default IDE service composition must not route those transport-based reads through `createBirdCoderSplitSdkApiClients()`.
- `scripts/default-ide-services-generated-app/backend-facade-contract.test.ts` is the executable contract for this adoption rule.
- `pnpm.cmd run check:release-flow` must execute that contract.
- Governance rule:
...
- `packages/sdkwork-birdcoder-infrastructure/src/services/sdkClients.ts` must not export a high-level `createBirdCoderSplitSdkApiClients()` wrapper.
...
- `scripts/app/backend-sdk-consumer-contract.test.ts` must consume `createBirdCoderSplitSdkApiClients({ appTransport, backendTransport })` directly.
- `scripts/no-app/backend-client-wrapper-contract.test.ts` is the executable contract for this hard-cut rule.
- `pnpm.cmd run check:release-flow` must execute that contract.
- Governance rule:
...
- `BirdCoderBackendSdkApiClient.listReleases()` is the explicit admin release catalog reader and must resolve against `GET /api/admin/v1/releases`.
...
- `scripts/default-ide-services-app-runtime-read-sdk-contract.test.ts` and `scripts/default-ide-services-core-read-service-contract.test.ts` are the executable contracts for this adoption rule.
...
- `scripts/app-runtime-write-sdk-client-contract.test.ts`, `scripts/default-ide-services-app-runtime-write-sdk-contract.test.ts`, and `scripts/api-backed-project-service-core-create-coding-session-contract.test.ts` are the executable contracts for this standard.
- Governance rule:
...
- `scripts/app-runtime-write-sdk-client-contract.test.ts`, `scripts/app-runtime-sdk-facade-governance-contract.test.ts`, and `scripts/api-backed-project-service-core-create-coding-session-turn-contract.test.ts` are the executable contracts for this standard.
...
- Admin deployment catalog truth must stay replayable across all current host modes:
  ...
  - legacy sqlite `kv_store` -> provider-side `deployment_records` materialization
  - direct sqlite provider -> `deployment_records`
- The shared generated app/backend facade now includes `listAdminDeployments()` and must remain the only promoted high-level client surface for representative admin deployment reads.
...
- The shared generated app/backend facade now includes `listPolicies()` and must remain the only promoted high-level client surface for representative admin policy reads.
