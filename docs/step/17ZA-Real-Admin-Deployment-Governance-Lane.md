...
Turn `GET /backend/v3/api/deployments` from a `not_implemented` placeholder into a real representative backend-surface governance read, then wire the admin deployment governance facade and first consumer path on top of the already-closed deployment authority model without mixing in policy governance.
...
- In-process app/backend transport now serves both app and admin deployment reads from the same repository-backed query layer without DTO drift.
...
1. This lane is serial because it changes representative admin route truth, shared app/backend facade governance, and admin deployment consumer adoption together.
