...
Close the first shared high-level representative app/backend facade on top of the release-backed generated client so infrastructure packages stop hand-assembling representative request paths.
...
- `CP17I-1` `@sdkwork/birdcoder-types` must own `createBirdCoderSplitSdkApiClients({ appTransport, backendTransport })`.
...
4. Expand the same shared-facade pattern to remaining shared `app / backend` consumers so host-local request assembly keeps shrinking.
