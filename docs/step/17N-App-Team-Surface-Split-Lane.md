...
Split runtime team reads from admin team reads on the shared app/backend facade so default IDE services stop routing workspace team catalogs through the backend surface.
...
- `CP17N-2` `BirdCoderBackendSdkApiClient.listGovernanceTeams()` must resolve against `/backend/v3/api/teams`.
...
2. Continue shared-facade adoption across remaining real `app / backend` transport consumers.
