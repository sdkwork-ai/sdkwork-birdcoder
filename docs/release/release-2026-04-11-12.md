...
- Stop-ship signals: engine catalog drift between `/engines`, `/engines/:engineKey/capabilities`, and `/models`; app-runtime SDK governance re-blocking promoted engine/model reads; or overview consumers rebuilding request paths outside `coreReadService`.
