...
- Promotes `listDeployments()` into the shared generated app/admin facade and shared deployment repository/query/transport boundary.
...
- Stop-ship signals: `/api/app/v1/deployments` regressing to `not_implemented`, `deployment_records` drifting between legacy/direct sqlite authority modes, or deployment reads bypassing the shared facade/service boundary.
...
- Writeback targets: `docs/架构/20-统一Rust-Coding-Server-API-协议标准.md`, `docs/架构/09-安装-部署-发布标准.md`, `docs/step/17-Coding-Server-Core-App-Admin-API与控制台实现.md`, `docs/step/17Z-Real-App-Deployment-Catalog-Lane.md`, `docs/step/17ZA-Real-Admin-Deployment-Governance-Lane.md`, `docs/prompts/反复执行Step指令.md`, `docs/release/releases.json`, and `docs/release/release-2026-04-11-16.md`.
