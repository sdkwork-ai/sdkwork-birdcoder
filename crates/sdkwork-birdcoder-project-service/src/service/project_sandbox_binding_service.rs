...
use crate::domain::sandbox_binding::{
    ...
    ProjectSandboxBindingAuditEntry, ProjectSandboxBindingIdempotency,
...
use crate::ports::sandbox_binding_repository::ProjectSandboxBindingRepository;
...
#[derive(Clone)]
pub struct ProjectSandboxBindingService {
    project_repository: Arc<dyn ProjectRepository>,
    binding_repository: Arc<dyn ProjectSandboxBindingRepository>,
}

impl ProjectSandboxBindingService {
    pub fn new(
        project_repository: Arc<dyn ProjectRepository>,
        binding_repository: Arc<dyn ProjectSandboxBindingRepository>,
    ) -> Self {
    ...
    pub async fn get_binding(
        ...
        self.binding_repository
            .get_sandbox_binding(context, project_id)
            .await?
            .ok_or_else(|| {
    ...
    pub async fn upsert_binding(
        ...
        context: &ProjectContext,
        project_id: &str,
        request: &UpsertProjectSandboxBindingRequest,
        expected_version: Option<i64>,
        idempotency_key: &str,
        audit_context: &ProjectSandboxBindingAuditContext,
    ) -> Result<ProjectSandboxBindingPayload, ProjectError> {
        validate_project_id(project_id)?;
        ...
        let key_hash = sha256_hash(idempotency_key.as_bytes());
        ...
        let binding = NewProjectSandboxBinding {
            project_id: project_id.to_owned(),
            sandbox_id: request.sandbox_id.clone(),
            root_entry_id: request.root_entry_id.clone(),
            logical_path,
            status: PROJECT_SANDBOX_BINDING_STATUS_ACTIVE.to_owned(),
            expected_version,
            idempotency: ProjectSandboxBindingIdempotency {
                operation: PROJECT_SANDBOX_BINDING_OPERATION_UPSERT.to_owned(),
                key_hash,
                request_fingerprint,
            },
        };
        let audit = ProjectSandboxBindingAuditEntry {
            action: "project.sandbox_binding.upsert".to_owned(),
            result: PROJECT_SANDBOX_BINDING_AUDIT_SUCCEEDED.to_owned(),
            reason_code: None,
        ...
        self.binding_repository
            .upsert_sandbox_binding(context, &binding, &audit)
            .await
    }
    ...
    pub async fn delete_binding(
        ...
        self.project_repository
            ...
            .await?;
        let audit = ProjectSandboxBindingAuditEntry {
            action: "project.sandbox_binding.delete".to_owned(),
            result: PROJECT_SANDBOX_BINDING_AUDIT_SUCCEEDED.to_owned(),
            reason_code: None,
            trace_id: audit_context.trace_id.clone(),
            redacted_metadata_json: json!({
        ...
        self.binding_repository
            .delete_sandbox_binding(context, project_id, expected_version, &audit)
            .await
    }
}
