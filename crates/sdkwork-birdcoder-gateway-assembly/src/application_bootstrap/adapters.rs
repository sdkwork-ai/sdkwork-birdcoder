...
use crate::bootstrap::code_engine_sandbox_policy::{
    resolve_code_engine_sandbox_policy, ResolvedCodeEngineSandboxPolicy, SandboxPolicyError,
};
...
impl CodeEngineProvider for KernelBridgeCodeEngineProvider {
    ...
    async fn execute_turn(
        ...
        let result = {
            ...
            let sandbox_policy = resolve_code_engine_sandbox_policy(ctx)
                .await
                .map_err(map_sandbox_policy_error)?;
...
fn build_turn_request(
    ...
    build_turn_request_with_runner_binding(
        pending,
        None,
        &ResolvedCodeEngineSandboxPolicy::all_drives(),
    )
}
...
fn build_turn_request_with_runner_binding(
    ...
    sandbox_policy: &ResolvedCodeEngineSandboxPolicy,
) -> Result<CodeEngineTurnRequestRecord, CodingSessionError> {
    let working_directory =
        resolve_authorized_working_directory(pending.working_directory.as_deref(), runner_binding)?;
    sandbox_policy
        .authorize_working_directory(&working_directory)
        .map_err(map_sandbox_policy_error)?;
    Ok(CodeEngineTurnRequestRecord {
        ...
        config: map_turn_config(pending.options.as_ref(), sandbox_policy),
    })
}
...
fn map_sandbox_policy_error(error: SandboxPolicyError) -> CodingSessionError {
    match error {
        SandboxPolicyError::Configuration(message) => CodingSessionError::Unavailable(format!(
            "Code-engine sandbox policy is invalid: {message}."
        )),
        SandboxPolicyError::Repository(message) => CodingSessionError::Repository(message),
        SandboxPolicyError::WorkingDirectoryDenied => CodingSessionError::Unavailable(
            "The selected project directory is not allowed by the effective code-engine sandbox policy."
                .to_owned(),
        ),
    }
}
...
fn map_turn_config(
    ...
    let mut config = CodeEngineTurnConfigRecord {
        ...
        sandbox_mode: Some(sandbox_policy.sandbox_mode().to_owned()),
        ..Default::default()
    };
...
mod tests {
    ...
    use super::{
        ...
        KernelBridgeCodeEngineProvider, ResolvedCodeEngineSandboxPolicy,
        SubmitApprovalDecisionInput, SubmitUserQuestionAnswerInput, TurnAdmissionPermits,
    ...
    fn build_turn_request_rejects_an_unsynchronized_runner_root() {
        ...
        let result = build_turn_request_with_runner_binding(
            &pending,
            Some(&runner_binding),
            &ResolvedCodeEngineSandboxPolicy::all_drives(),
        );
