...
impl CommerceRepository for SqliteCommerceRepository {
    ...
    async fn create_order(
        ...
        let id = inserted_row_id(&id_row).map_err(|e| e.to_string())?;
    ...
    async fn create_pending_payment(
        ...
        self.find_payment(tenant_id, user_id, payment_id)
            .await?
            .ok_or_else(|| "created pending payment could not be loaded".to_string())
    }
    ...
    async fn finalize_pending_payment(
        ...
        self.find_payment(tenant_id, user_id, payment_id)
            .await?
            .ok_or_else(|| "confirmed payment could not be loaded".to_string())
    }
}
