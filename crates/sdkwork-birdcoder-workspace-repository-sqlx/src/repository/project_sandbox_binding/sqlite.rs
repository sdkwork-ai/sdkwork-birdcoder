...
SET sandbox_id = ?, root_entry_id = ?, logical_path = ?, status = ?,
...
WHERE id = ? AND tenant_id = ? AND organization_id = ? AND project_id = ?
  ...
  AND EXISTS (
      ...
      WHERE p.id = studio_project_sandbox_binding.project_id
        ...
        AND p.owner_user_id = ? AND p.is_deleted = 0
  )
"#,
        )
...
pub(super) async fn delete(
    ...
    let result = sqlx::query(
        r#"
UPDATE studio_project_sandbox_binding
SET status = ?, updated_by_user_id = ?, is_deleted = 1,
...
async fn ensure_project_owner(
    ...
    let found = sqlx::query_scalar::<_, i64>(
        "SELECT 1 FROM studio_project WHERE id = ? AND tenant_id = ? AND organization_id = ? AND owner_user_id = ? AND is_deleted = 0",
    )
    .bind(scope.project_id)
    .bind(scope.tenant_id)
    .bind(scope.organization_id)
...
WHERE tenant_id = ? AND organization_id = ? AND project_id = ?
  AND subject_user_id = ? AND operation_kind = ? AND idempotency_key_hash = ?
  AND expires_at <= ?
"#,
    )
    .bind(scope.tenant_id)
...
fn map_row(row: SqliteRow) -> Result<ProjectSandboxBindingPayload, ProjectError> {
    Ok(StoredBinding {
