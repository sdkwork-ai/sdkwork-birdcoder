...
export type WorkbenchCodeEngineModelSource = 'builtin';
...
export function hasWorkbenchCodeModel(
  ...
  return definition.models.some((model) => model.id === id);
}
...
export function normalizeWorkbenchCodeEngineSettingsMap(
  ...
  for (const engine of WORKBENCH_CODE_ENGINES) {
    ...
    const defaultModelId = normalizeWorkbenchCodeModelId(engine.id, candidate);

    if (!includeDefaults && defaultModelId === engine.defaultModelId) {
      continue;
    }
...
export function resolveWorkbenchCodeEngineSelectedModelId(
  ...
  const configuredModelId = isRecord(settings)
    ? String(settings.defaultModelId ?? settings.selectedModelId ?? '').trim()
    : '';

  return normalizeWorkbenchCodeModelId(
    normalizedEngineId,
    explicit || configuredModelId,
    carrier,
  );
}
