...
#[tauri::command]
async fn project_device_mount_find(
    app: tauri::AppHandle,
    project_id: String,
    owner_keys: Vec<String>,
) -> Result<Option<host::ProjectDeviceMountEntry>, String> {
    host::project_device_mount_find(app, project_id, owner_keys).await
}
...
#[tauri::command]
async fn fs_read_image_preview(root_path: String, relative_path: String) -> Result<String, String> {
    host::fs_read_image_preview(root_path, relative_path).await
}
