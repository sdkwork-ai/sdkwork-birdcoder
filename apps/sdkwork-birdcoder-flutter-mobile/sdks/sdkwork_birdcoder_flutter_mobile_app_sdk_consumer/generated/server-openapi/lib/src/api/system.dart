...
class SystemApi {
  ...
  Future<BirdCoderChatConversationSummaryListEnvelope?> chatConversationsList([int? page, int? pageSize]) async {
  ...
  Future<void> chatConversationsDelete(String conversationId) async {
    await _client.delete(ApiPaths.appPath('/chat/conversations/${serializePathParameter(conversationId, const PathParameterSpec('conversationId', 'simple', false))}'));
  }

  /// List chat messages
  Future<BirdCoderChatMessageSummaryListEnvelope?> chatConversationsMessagesList(String conversationId, [int? page, int? pageSize]) async {
    final query = buildQueryString([
      QueryParameterSpec('page', page, 'form', true, false, null),
      QueryParameterSpec('page_size', pageSize, 'form', true, false, null)
    ]);
