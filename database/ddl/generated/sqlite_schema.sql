...
-- Generated at: 2026-07-16T05:12:46.256Z
...
-- baseline source: ddl/baseline/sqlite/0001_birdcoder_baseline.sql
...
CREATE TABLE IF NOT EXISTS ai_coding_session (
    ...
    -- Legacy pre-release session rows may lack a binding. Application code
    -- permits reads but rejects execution until an explicit location exists.
...
CREATE INDEX IF NOT EXISTS idx_ai_coding_session_tenant_project_runtime_location_updated
ON ai_coding_session(tenant_id, project_id, runtime_location_id, updated_at);
...
WHERE native_session_id IS NOT NULL AND is_deleted = 0;
...
-- ============================================================
-- Studio Project Runtime Location tables (tenant-scoped)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS studio_project_runtime_location_preference (
    ...
    runtime_location_id TEXT NOT NULL REFERENCES studio_project_runtime_location(id),
    version INTEGER NOT NULL DEFAULT 0,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    is_deleted INTEGER NOT NULL DEFAULT 0
...
-- The key is a SHA-256 digest of the HTTP Idempotency-Key. The raw header,
-- plaintext path, ciphertext, and path fingerprint are intentionally absent.
...
-- Append-only audit facts. Do not add mutable lifecycle columns or sensitive
-- location material to this table.
...
-- Serves owner-scoped realtime replay and MAX(sequence_no) allocation while
-- enforcing one active durable sequence per owner/session stream.
...
CREATE INDEX IF NOT EXISTS idx_studio_project_runtime_location_project_created
ON studio_project_runtime_location(tenant_id, organization_id, project_id, is_deleted, created_at, id);

CREATE INDEX IF NOT EXISTS idx_studio_project_runtime_location_project_target
ON studio_project_runtime_location(tenant_id, organization_id, project_id, runtime_target_id, is_deleted);
...
CREATE INDEX IF NOT EXISTS idx_ops_project_runtime_location_audit_scope_time
ON ops_project_runtime_location_audit(tenant_id, organization_id, project_id, runtime_location_id, occurred_at);
