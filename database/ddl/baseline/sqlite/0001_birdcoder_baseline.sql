-- SDKWork BirdCoder greenfield baseline (SQLite)
...
CREATE TABLE IF NOT EXISTS studio_workspace (
    ...
    CONSTRAINT chk_studio_workspace_code_nonblank CHECK (length(trim(code)) BETWEEN 1 AND 96),
    CONSTRAINT chk_studio_workspace_name_nonblank CHECK (length(trim(name)) BETWEEN 1 AND 160)
);
...
CREATE TABLE IF NOT EXISTS studio_project (
    ...
    updated_at TEXT NOT NULL,
    is_deleted INTEGER NOT NULL DEFAULT 0 CHECK (is_deleted IN (0, 1)),
    CONSTRAINT chk_studio_project_code_nonblank CHECK (length(trim(code)) BETWEEN 1 AND 96),
    CONSTRAINT chk_studio_project_name_nonblank CHECK (length(trim(name)) BETWEEN 1 AND 160),
...
WHERE is_deleted = 0 AND default_agent_project_id IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS studio_project_document_binding (
    ...
    updated_at TEXT NOT NULL,
    is_deleted INTEGER NOT NULL DEFAULT 0 CHECK (is_deleted IN (0, 1)),
    CONSTRAINT chk_studio_project_document_id_nonblank CHECK (length(trim(document_id)) BETWEEN 1 AND 160),
    CONSTRAINT chk_studio_project_document_kind_nonblank CHECK (length(trim(binding_kind)) BETWEEN 1 AND 64)
);
...
CREATE TABLE IF NOT EXISTS studio_project_runtime_location (
    ...
    terminal_available INTEGER NOT NULL DEFAULT 0 CHECK (terminal_available IN (0, 1)),
    git_available INTEGER NOT NULL DEFAULT 0 CHECK (git_available IN (0, 1)),
    build_available INTEGER NOT NULL DEFAULT 0 CHECK (build_available IN (0, 1)),
    filesystem_available INTEGER NOT NULL DEFAULT 0 CHECK (filesystem_available IN (0, 1)),
    ...
    updated_at TEXT NOT NULL,
    is_deleted INTEGER NOT NULL DEFAULT 0 CHECK (is_deleted IN (0, 1)),
    CONSTRAINT chk_studio_project_runtime_target_id_nonblank CHECK (length(trim(runtime_target_id)) BETWEEN 1 AND 160),
    CONSTRAINT chk_studio_project_runtime_display_name_nonblank CHECK (length(trim(display_name)) BETWEEN 1 AND 160),
    CONSTRAINT chk_studio_project_runtime_ciphertext_nonblank CHECK (length(encrypted_absolute_path) > 0),
    CONSTRAINT chk_studio_project_runtime_fingerprint CHECK (
        length(path_fingerprint) = 64 AND path_fingerprint NOT GLOB '*[^0-9a-f]*'
    )
);
...
CREATE TABLE IF NOT EXISTS studio_project_runtime_location_idempotency (
    ...
    CONSTRAINT chk_studio_project_runtime_location_idempotency_hash CHECK (
        length(idempotency_key_hash) = 64 AND idempotency_key_hash NOT GLOB '*[^0-9a-f]*'
    ),
    CONSTRAINT chk_studio_project_runtime_location_request_hash CHECK (
...
CREATE TABLE IF NOT EXISTS studio_project_sandbox_binding (
    ...
    updated_at TEXT NOT NULL,
    is_deleted INTEGER NOT NULL DEFAULT 0 CHECK (is_deleted IN (0, 1)),
    CONSTRAINT chk_studio_project_sandbox_id_nonblank CHECK (length(trim(sandbox_id)) BETWEEN 1 AND 160),
    CONSTRAINT chk_studio_project_sandbox_root_entry_id_nonblank CHECK (length(trim(root_entry_id)) BETWEEN 1 AND 160),
    CONSTRAINT chk_studio_project_sandbox_logical_path CHECK (
        logical_path = '' OR (
            instr('/' || logical_path || '/', '/../') = 0
...
CREATE TABLE IF NOT EXISTS studio_project_sandbox_binding_idempotency (
    ...
    CONSTRAINT chk_studio_project_sandbox_binding_idempotency_hash CHECK (
        length(idempotency_key_hash) = 64 AND idempotency_key_hash NOT GLOB '*[^0-9a-f]*'
    ),
    CONSTRAINT chk_studio_project_sandbox_binding_request_hash CHECK (
        length(request_fingerprint) = 64 AND request_fingerprint NOT GLOB '*[^0-9a-f]*'
    )
);
